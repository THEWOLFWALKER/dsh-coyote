...
function deepMerge(target, source) {
	...
	for (const key in source) if (Object.prototype.hasOwnProperty.call(source, key)) {
		...
		if (Array.isArray(targetValue) && Array.isArray(sourceValue)) result[key] = [...targetValue, ...sourceValue];
		else if (typeof targetValue === "object" && targetValue !== null && typeof sourceValue === "object" && sourceValue !== null && !Array.isArray(targetValue) && !Array.isArray(sourceValue)) result[key] = deepMerge({ ...targetValue }, sourceValue);
...
async function bundle(options) {
	...
	const mergedInputOptions = deepMerge({
		input: options.path,
		platform: "node",
	...
	if (tsconfig) mergedInputOptions.tsconfig = tsconfig;
	const bundle = await rolldown(mergedInputOptions);
	const mergedOutputOptions = deepMerge({
		format: "esm",
		codeSplitting: false,
		keepNames: true,
